...
class Agent:
	...
	def is_label_in_arguments_base(self, argument_label):
		arguments = self.get_all_arguments_labels()
	...
	def get_all_arguments_labels(self):
		arguments_labels = set()
		epistemic_labels = self.get_epistemic_base_labels()
		practical_labels = self.get_practical_base_labels()

		arguments_labels = arguments_labels.union(epistemic_labels).union(practical_labels)
	...
	def get_certain_attacked_labels_set_from_argument_label(self, argument_label):
		...
		if "ae" in argument_label:
			epistemic_base = self.get_epistemic_base_structures()
			practical_base = self.get_practical_base_structures()
			
			for argument_structure in epistemic_base:
				if argument_label in argument_structure[1] and self.is_label_in_arguments_base(argument_label) == True:
					attacked_set.add(argument_structure[0])
	
			for argument_structure in practical_base:
				if argument_label in argument_structure[3] and self.is_label_in_arguments_base(argument_label) == True:
					attacked_set.add(argument_structure[0])
	...
	def get_maximal_cfs(self):
		all_arguments_labels = list(self.get_all_arguments_labels())
	...
	""" 
	def compute_cfs_a(self):
		args = self.get_all_arguments_labels()
		pwr = powerset(args)

		sU = self.get_undercuts()
		sA = self.get_attacks()
	...
	def compute_admissibility(self, cfs):
		...
		if len(cfs) > 0:
			...
			for cfset in cfs:
				...
				#print("cfs", cfset)
				if cfset != set():
					for element in cfset: # reviso si cada elemento es defendido por el propio cfset
						comp = cfset.difference(element)
						if self.__is_defendedBySet(element, comp) == False:
							status = False
							break
