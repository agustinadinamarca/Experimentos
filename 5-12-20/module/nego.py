...
def firstMove(agents, agSp, Dialogue, alternativasRestantes):
	...
	# añado move
	Dialogue.add_wfMove(M0) # REMOVER, YA NO TIENE SENTIDO
	# postconditions
...
def negociation(agents, X):
	...
	while len(Xcurrent) > 0 and agreement == False and count < laa: # en cada iteracion empieza un dialogo nuevo
		...
		while status == True:
			...
			result = 0
			# RESULT -ACCEPTX
			if nm[0].moveAct == "AcceptX" and moveCount >= laa:
				c=0
				
				for agent in listAg:
					if xcurr in agent.cs.S:
						c+=1
				if c==laa:
					result = "Success"
			...
			if nm[0].moveAct == "Argue":
				count1 = 0
				for ag in listAg:
					if ag.name != firstS:
						#if xCurrent == ag.candidatesDecisionPrefOrder()[1][0]:
						if ag.compAltAcc(xcurr)==True:
							count1+=1
				if count1 == laa - 1:
					result = "Success"
