...
def pafs_negotiations(fname, number_agents, step, M, taf_agents, resource_boundness_densities, taf_agents_final):
	
	data_to_save = open(fname, "w")
	data_to_save.write("Argumentation Problem")
	data_to_save.write("Decision-Making Algorithm\n")
	# nombres de las columnas
	data_to_save.write("num_agents_R num_agents_RI time_neg_mean time_neg_std FP_mean FP_std FN_mean FN_std TP_mean TP_std TN_mean TN_std bullshit_mean bullshit_std gwc_mean gwc_std lwc_mean lwc_std\n")
	#data_to_save.close()
	...
	for semantics_list in ssc:
		...
		for i in range(M):
			...
			#	print("OK1")
			paf_agents = create_paf_agents(taf_agents, resource_boundness_densities, semantics_list)
