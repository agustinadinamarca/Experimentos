...
def exp(numAg, numMaxGoals, numAlt, numMaxAp, numMaxEp):
    ...
    while sol != 1:
        agents = sin.createAgents(numAg=numAg, numMaxGoals=numMaxGoals, numAlt=numAlt, numMaxAp=numMaxAp, limMaxSup=2, numMaxEp=numMaxEp)
        ...
        fileO = open("argsAcc-R*.txt", "w")
        fileO.write(str(len(agents))+"\n")
        for ag in agents:
            fileO.write("#\n"+ag.name+"\n")
            for e in ag._Agent__accArgs:
                if e.t == "P":
                    fileO.write("PRO "+e.x+" "+e.C+" ")
                    for s in e.S:
                        if len(s) == 2:
                            fileO.write(str(2)+" "+str(s[0])+" "+str(s[1])+" ")
                        else:
                            fileO.write(str(3)+" "+str(s[0])+" "+str(s[1])+" "+str(s[2])+" ")
                    fileO.write("\n")
                elif e.t == "C":
                    fileO.write("CON "+e.x+" "+e.C+" ")
                    for s in e.S:
                        if len(s) == 2:
                            fileO.write(str(2)+" "+str(s[0])+" "+str(s[1])+" ")
                        else:
