...
class Agent:
	def __init__(self, name, K, G, X, Ae, Ap):
		self.name = name # un label con el nombre del agente
		self.numAttacksToProcessByTurn = 0
	...
	def __get_AttackersOrUndercutersOfArg(self, arg):
		lst = set()
		for pair in self.__stronglyUndercuts:
			if pair[1].t == "E" and arg.t == "E":
				if pair[1].H == arg.H and pair[1].h == arg.h:
					lst.add(pair[0])
		for pair in self.__stronglyAttacks:
	...
	def __is_defendedBySet(self, arg, E):
		Bs = self.__get_AttackersOrUndercutersOfArg(arg)
		lst = []
		if len(E) >= 0:
			for element in Bs:
				at = self.__get_AttackersOrUndercutersOfArg(element)
	...
	def get_rejectedArgs(self):
		lst=set()
		for pair in self.__stronglyAttacks:
			if inTuple(pair[0], self.__accArgs):
				lst.add(pair[1])
		for pair in self.__stronglyUndercuts:
			if inTuple(pair[0], self.__accArgs):
				lst.add(pair[1])
	...
	def compute_cfs(self):
		...
		if len(self.__stronglyUndercuts) > 0:
			for x in self.__stronglyUndercuts:
			   x1 = x[0]
			   x2 = x[1]
			   todelete = []
			   for i in pwr:
		...
		if len(self.__stronglyAttacks) > 0:
			for x in self.__stronglyAttacks:
				x1 = x[0]
				x2 = x[1]
				todelete = []
				for i in pwr:
...
###############################
	...
	def argExtIsAcceptable(self, arg):
		...
		if is_defendedBySet(arg, self._Agent__accArgs, AePrima, ApPrima) == True:
			return True
		else:
			return False
...
def get_AttackersOrUndercutersOfArg_new(arg, AePrima, ApPrima):
		stronglyUndercuts = get_stronglyUndercuts_new(AePrima)
		stronglyAttacks = get_stronglyAttacks_new(AePrima, ApPrima)
		
		lst = set()
		for pair in stronglyUndercuts:
			if pair[1].t == "E" and arg.t == "E":
				if pair[1].H == arg.H and pair[1].h == arg.h:
					lst.add(pair[0])
		for pair in stronglyAttacks:
			if pair[1].t == "P" and arg.t == "P":
				if pair[1].S == arg.S and pair[1].C == arg.C and pair[1].x == arg.x:
					lst.add(pair[0])
		for pair in stronglyAttacks:
...
def is_defendedBySet(arg, E, AePrima, ApPrima):
		Bs = get_AttackersOrUndercutersOfArg_new(arg, AePrima, ApPrima)
		lst = []
		if len(E) >= 0:
			for element in Bs:
				at = get_AttackersOrUndercutersOfArg_new(element, AePrima, ApPrima)
				if intersection(at, E):
					lst.append(True)
				else:
					lst.append(False)
