...
global mainName
global file11
global stepPropSem # step para evaluar la proporcion de agentes a estudiar [0, N]
#############################################################
...
def exp1():
    ...
    # me quedo con una selección según step...
    RR = []
    for i in range(0, len(SSet), stepPropSem):
        RR.append(SSet[i])
    if SSet[len(SSet)-1] not in RR:
        RR.append(SSet[len(SSet)-1])
    ...
    while countat2 < len(val2):
      ...
      while countat1 < len(val1):
          ...
          for S in RR: # [RI, RI, R]
              ...
              # ACÁ DEBERÍA HACER LO DE DEEPCOPY
              Ag = deepcopy(Agg)
